-- Migration: Import Complete Attributes from CSV and Update Roles
-- Version: 1.0
-- Date: 2025-10-09
-- Dependencies: All previous currency migrations

-- ===========================================
-- CLEAN UP EXISTING ATTRIBUTES (Keep system data)
-- ===========================================

-- Drop foreign key constraints temporarily
ALTER TABLE public.game_accounts DROP CONSTRAINT IF EXISTS game_accounts_league_attribute_id_fkey;
ALTER TABLE public.currency_inventory DROP CONSTRAINT IF EXISTS currency_inventory_currency_attribute_id_fkey;
ALTER TABLE public.currency_inventory DROP CONSTRAINT IF EXISTS currency_inventory_game_account_id_fkey;
ALTER TABLE public.currency_transactions DROP CONSTRAINT IF EXISTS currency_transactions_league_attribute_id_fkey;
ALTER TABLE public.currency_transactions DROP CONSTRAINT IF EXISTS currency_transactions_currency_attribute_id_fkey;
ALTER TABLE public.currency_transactions DROP CONSTRAINT IF EXISTS currency_transactions_game_account_id_fkey;

-- Now safely delete all dependent records and attributes
...
DELETE FROM public.attributes WHERE type IN (
  'LEAGUE_POE1', 'LEAGUE_POE2', 'LEAGUE_D4',
  'POE1_CURRENCY', 'POE2_CURRENCY', 'D4_CURRENCY'
);

-- Re-add foreign key constraints
ALTER TABLE public.game_accounts
ADD CONSTRAINT game_accounts_league_attribute_id_fkey
FOREIGN KEY (league_attribute_id) REFERENCES public.attributes(id) ON DELETE CASCADE;

ALTER TABLE public.currency_inventory
ADD CONSTRAINT currency_inventory_currency_attribute_id_fkey
FOREIGN KEY (currency_attribute_id) REFERENCES public.attributes(id) ON DELETE CASCADE;

ALTER TABLE public.currency_inventory
ADD CONSTRAINT currency_inventory_game_account_id_fkey
FOREIGN KEY (game_account_id) REFERENCES public.game_accounts(id) ON DELETE CASCADE;
...
ALTER TABLE public.currency_transactions
ADD CONSTRAINT currency_transactions_currency_attribute_id_fkey
FOREIGN KEY (currency_attribute_id) REFERENCES public.attributes(id) ON DELETE CASCADE;

ALTER TABLE public.currency_transactions
ADD CONSTRAINT currency_transactions_game_account_id_fkey
FOREIGN KEY (game_account_id) REFERENCES public.game_accounts(id) ON DELETE CASCADE;

-- ===========================================
-- INSERT ROLES FROM EXISTING ROLES TABLE
-- ===========================================

-- Insert roles based on existing roles table structure (using only valid enum values)
INSERT INTO public.roles (code, name, description, sort_order, is_active) VALUES
('admin', 'Administrator', 'System administrator with full access', 1, true),
('manager', 'Manager', 'Manager with business oversight', 2, true),
('mod', 'Moderator', 'Content moderation role', 3, true),
('trader1', 'Trader Level 1', 'Junior trader role', 4, true)
...
-- POE1 Currency Attributes (45 types)
INSERT INTO public.attributes (code, name, type, sort_order, is_active) VALUES
...
-- POE2 Currency Attributes (44 types)
INSERT INTO public.attributes (code, name, type, sort_order, is_active) VALUES
