-- FIX: Pool lookup WITHOUT channel_id and cost_currency
-- Pool uniqueness: game_code + currency_attribute_id + server_attribute_code + game_account_id ONLY
-- All pools store cost in VND
-- PURCHASE orders with non-VND cost_currency are converted using exchange_rates
...
DECLARE
    ...
    v_cost_amount_vnd DECIMAL;  -- Cost converted to VND
...
BEGIN
    ...
    -- ============================================
    -- All pools store cost in VND, so convert if order uses different currency
    IF v_cost_currency = 'VND' THEN
        v_cost_amount_vnd := v_cost_amount;
    ELSE
        -- Get CURRENT exchange rate at delivery time (NOW)
        -- Use the rate that is active right now, not historical rate
        SELECT rate INTO v_exchange_rate
        FROM exchange_rates
        WHERE from_currency = v_cost_currency
          AND to_currency = 'VND'
          AND is_active = true
        ...
        IF v_exchange_rate IS NULL OR v_exchange_rate = 0 THEN
            RETURN QUERY SELECT FALSE,
                format('Cannot find exchange rate from %s to VND for order %s',
                       v_cost_currency,
                       COALESCE(v_order.order_number, p_order_id::TEXT))::TEXT,
                NULL::JSONB;
            RETURN;
        END IF;

        v_cost_amount_vnd := v_cost_amount * v_exchange_rate;
    END IF;
    ...
    -- ============================================
    -- FIND EXISTING POOL WITHOUT channel_id
    -- ============================================
    -- Pool uniqueness: game_account_id + currency_attribute_id + game_code + server_attribute_code
    -- NOT including channel_id or cost_currency (always VND)
    ...
    ELSE
        v_old_quantity := 0;
        v_old_average_cost := 0;
    END IF;

    -- ============================================
    -- UPDATE OR CREATE POOL (always in VND)
    -- ============================================
    IF v_existing_pool_found THEN
        ...
        IF v_new_quantity > 0 THEN
            v_new_average_cost := (
                (v_old_quantity * v_old_average_cost) + v_cost_amount_vnd
            ) / v_new_quantity;
        ELSE
            v_new_average_cost := v_cost_amount_vnd / v_order.quantity;
        END IF;

        UPDATE inventory_pools SET
            quantity = v_new_quantity,
            average_cost = v_new_average_cost,
            cost_currency = 'VND',  -- Always VND
            last_updated_at = NOW(),
    ...
    INSERT INTO currency_transactions (
        ...
        v_order.currency_attribute_id,
        v_order.quantity,
        v_cost_amount_vnd / v_order.quantity,  -- Unit price in VND
        'VND',  -- Transaction stores VND
        p_order_id,
        v_final_proofs,
        format('Purchase receiving confirmation - Order: %s | Original cost: %s %s (%s %s VND) | Rate: %s',
               COALESCE(v_order.order_number, p_order_id::TEXT),
               v_cost_amount::TEXT,
               v_cost_currency,
               v_cost_amount_vnd::TEXT,
               'VND',
               COALESCE(v_exchange_rate, 1)::TEXT
        ),
        v_user_id,
        v_old_quantity,
        v_new_quantity
    ...
    RETURN QUERY SELECT TRUE,
        format('Purchase order receiving confirmed! %s added to inventory (Pool: %s, Before: %s, After: %s) | Cost: %s VND/unit',
               v_order.quantity,
               v_inventory_pool.id::TEXT,
               v_old_quantity::TEXT,
               v_new_quantity::TEXT,
               v_new_average_cost::TEXT
        ),
        jsonb_build_object(
            ...
            'original_cost', jsonb_build_object(
                'amount', v_cost_amount,
                'currency', v_cost_currency
            ),
            'converted_cost', jsonb_build_object(
                'amount_vnd', v_cost_amount_vnd,
                'exchange_rate', COALESCE(v_exchange_rate, 1)
            ),
            'new_average_cost_vnd', v_new_average_cost,
...
SELECT
    ...
    'UPDATED' as status,
    'Pool lookup without channel_id, all pools in VND with exchange rate conversion' as notes
FROM pg_proc
